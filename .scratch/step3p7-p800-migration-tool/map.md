...
环境和 Agent 按需选择其余变量的规则也已封存。revision 6 正式 runbook 已生成并完成 SOURCE 审查；
下一步转到 CUDA 机器，严格执行
`model-adaptation/references/formal-cuda-capture-r6.md`：先执行只检查 Torch、
TP8、BF16、固定 SGLang 和插件入口的环境 `preflight`，通过后在同一 reservation
内完成唯一正式 Session、全部 Golden self-replay 和 Handoff Bundle build/verify。
算子 Hook、shape 去重和 self-replay 已重新归类为历史采集适配器验证。
...
- [构建缺口清单驱动的多 Golden Handoff Bundle](issues/28-build-gap-driven-multi-golden-bundle.md)：Manifest v2 从不可变 Scan Run 得到有序缺口集合，要求每项恰好一个 SEALED Golden 和 record-samples Run，且全部 Golden 来自同一正式 Session/采集进程；preflight 配置会被拒绝，Session 请求与正式结果必须绑定 Scan 和全部 Golden。包内保存 Scan、Session 配置、正式结果、全部 Golden 与完整文件摘要。增减缺口只改变 Scan 和证据输入，不修改 bundle 工具中的算子常量。正式采集后由同一 CUDA Agent 连续审查、构建、校验，最后一次性回传。
