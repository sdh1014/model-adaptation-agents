...
- [构建缺口清单驱动的多 Golden Handoff Bundle](issues/28-build-gap-driven-multi-golden-bundle.md)：Manifest v2 从不可变 Scan Run 得到有序缺口集合，要求每项恰好一个 SEALED Golden 和 record-samples Run，并把 Scan、全部 Golden 与完整文件摘要一起封包；增减缺口只改变 Scan 和证据输入，不修改 bundle 工具中的算子常量。正式采集后由同一 CUDA Agent 连续审查、构建、校验，最后一次性回传。
