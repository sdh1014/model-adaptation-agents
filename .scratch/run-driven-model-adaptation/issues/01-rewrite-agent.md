...
- 把五个历史 gap 全部加入 Operator Verification Queue。
- 用公开 Skill/Spec 行为测试锁定新接口并清理残留引用。

## Comments

- 2026-07-20：用户明确要求直接改写，并把之前 gap 算子全部纳入测试。
- 2026-07-20：公开 Skill/Spec seam 的 5 个行为测试通过；旧 Python 栈、专用
  runbook、旧 tracker/prototype 和旧实现测试已删除，五个历史 gap 全部以
  `PENDING` 进入 revision 7 队列。
