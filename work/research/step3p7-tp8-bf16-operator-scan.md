...
- 加载后配置证据：`runs/scan-002/checkpoint-config-summary.json`，由固定 CUDA revision 的 `Step3p7Config(**raw_config)` 加载，记录 Transformers 版本、配置类锚点、原始配置摘要和实际激活层。
...
MoE 也不在命令中显式指定 backend。`ServerArgs` 的源码默认是 `moe_a2a_backend=none`、`moe_runner_backend=auto`：`python/sglang/srt/server_args.py:1601-1623`。Step-3.7 没有改写这两个值，因此 target MoE 走 `Step3p5MoEMLP.forward_normal`：`python/sglang/srt/models/step3p5.py:177-193`；BF16 未量化路径的 `auto` 在 `UnquantizedFusedMoEMethod.create_moe_runner` 中落为 Triton：`python/sglang/srt/layers/quantization/unquant.py:410-421`。首次 CUDA 启动日志仍要记录实际值，防止环境变量或平台能力改变源码默认。
...
5. 命令不传 speculative 调优参数时，源码默认得到 `num_steps=3`、`topk=1`、`num_draft_tokens=4`：`python/sglang/srt/arg_groups/speculative_hook.py:316-329,512-542`。TP worker 因而创建 `draft_model_idx=0..2` 三个 runner：`python/sglang/srt/managers/tp_worker.py:375-423`。
6. 三个 `Step3p5MTP` 模块都用源码固定的 config `layer_id=45` 决定 attention/MLP/limit 分支，但 loader 按 `draft_model_idx` 分别选择 checkpoint 的 45、46、47 层权重：`python/sglang/srt/models/step3p5_mtp.py:77-85,203-215`。

所以“命令不额外开启 MTP”只描述 CLI；运行时的 EAGLE draft 仍由 `Step3p5MTP` 实现。三个 runner 的权重不同，但 operator 分支相同，因此只形成一套 draft Semantic Operator 清单。
...
- EAGLE draft 有三个 runner，分别加载 checkpoint layer 45-47；当前实现都用 config layer 45 的分支，为 sliding attention + Dense MLP。
...
入口依据：`python/sglang/srt/models/step3p7.py:47-74,136-152`；纯文本 embedding 先于 language model forward 的顺序见 `python/sglang/srt/managers/mm_utils.py:1023-1145`；DecoderLayer 内的 norm、attention、Dense/MoE 和 residual 路径见 `python/sglang/srt/models/step3p5.py:480-665`。

### Draft

```text
Step3p5MTP.forward（draft_model_idx 0..2）
  -> Step3p5AMultiTokenPredictor.forward
  -> Step3p5DecoderLayer.forward（分支固定 config layer 45；权重分别来自 45..47）
...
每个 operator 从上述入口到具体计算边界的逐项调用链保存在 `runs/scan-002/result.json` 的 `operators[].call_chains`；这里不重复展开 13 组链路。
...
| `linear.bf16` | qkv/o、Dense/shared MLP、draft `eh_proj` | activation + BF16 weight -> output | `linear.py:466-478` 调用 `UnquantizedLinearMethod.apply`，后者在 `quantization/unquant.py:110-162` 使用 `F.linear` | 固定 Kunlun revision 的相同 `linear.py:466-478` 与 `quantization/unquant.py:110-162`；draft `eh_proj` 见 `step3p5_mtp.py:79-109` | 需要权重，当前不采集；没有发现专用语义缺口 | `READY` |
...
| `activation.step_swiglu_with_limit` | target shared expert layer 43/44，limit=16 | gate_up + scalar limit -> clamped SwiGLU output；无权重 | helper `models/step3p5_ops.py:5-13`，调用点 `step3p5.py:95-104` | 固定 Kunlun revision 存在相同 helper 与调用点，但没有专用 replacement | 可采输入和 CUDA 输出，最多三种真实 shape；Demo 首选候选 | `CAPTURE_REQUIRED` |
| `moe.router_fp32` | target MoE layer 3-44 | hidden states + BF16 router weight -> FP32 logits | `step3p5.py:205-221` 的 FP32 `torch.matmul` | 固定 Kunlun revision 的同路径 `python/sglang/srt/models/step3p5.py:205-221` | 需要权重，当前不采集；实现链完整 | `READY` |
...
| `moe.bf16_clamped` | target MoE layer 43/44，expert limit=7；默认 A2A 为 `none` | 同上，且第一段激活必须 clamp | limit 从 `step3p5.py:134-158` 经 `fused_moe_triton/layer.py:250-270` 进入 runner；BF16 `auto` 默认落到 Triton 并传 `gemm1_limit`：`quantization/unquant.py:410-421`、`moe_runner/triton.py:145-165` | Kunlun replacement 在 `hooks/layers/quantization/unquant.py:122-125` 只调用普通 `kunlun_ops.swiglu`，未读取 limit | 已确认语义缺口；现有边界包含禁止保存的 expert weights | `NEEDS_HUMAN` |
...
| `logits.lm_head` | target 与 draft；不返回 logprobs | final hidden + BF16 vocab weight -> TP logits | `logits_processor.py:304-381,830-917` | 固定 Kunlun revision 的同路径 `python/sglang/srt/layers/logits_processor.py:304-381,830-917` 与 Kunlun TP group | 需要权重，当前不采集；实现链完整 | `READY` |
...
| `moe.bf16_clamped` | 已确认 Kunlun 忽略 limit；首次日志同时确认 CUDA MoE runner | 不纳入 | 当前可替换边界包含 expert weights |
...
- CUDA 与 Kunlun 两个固定 revision 上均实跑 `Ran 2 tests, OK`，两侧 helper patch 完全一致。

这个 helper 只建立采集和替换入口，不算 Operator Gap，也不消耗 repair attempt。

## 扫描结论

BF16 分支已经闭合了旧 INT8 扫描中的 checkpoint/quantization 不确定性，并确认 EAGLE 仍会进入 `Step3p5MTP`。当前完整清单发现三个有明确代码缺口、但因禁止保存权重或 bias 而无法按现行边界重放的算子；attention 的 CUDA 默认 backend 需要实际启动日志补证，源码默认解析为 Triton 的 CUDA MoE runner 也在同一日志中确认。特殊 SwiGLU 是唯一可以直接进入现行采集边界的候选，但按既定停止规则，在上述 `NEEDS_HUMAN` 被人处理前不能消耗唯一 CUDA Session。
