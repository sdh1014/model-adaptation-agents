...
def validate_signature_group(
    ...
    if not isinstance(values, dict) or list(values) != list(expected_names):
    ...
    *,
    ...
    for field, expected in checks.items():
        if result.get(field) != expected:
            if field == "sample_files_sha256":
                raise ToolError(
                    f"record-samples result for {operator_id!r} "
                    "does not match sample-files.json"
                )
            raise ToolError(
...
def validate_gap_bundle(
    ...
    not_permitted = [
        path
        for path in paths
        if path not in {"migration-spec.md", "scan-result.json"}
