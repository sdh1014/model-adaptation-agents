...
当前 Contract 是 revision 5，`scan-005` 在扫描完成后选择
`sgl_kernel.gemma_rmsnorm`。对应的 capture/replay adapter 尚未实现，所以现在
不能执行 CUDA preflight。

仓库中现有 `Step3p5MLP.forward` adapter 和旧命令属于 revision 4 历史方案。不要
把 `scan-005`、`sgl_kernel.gemma_rmsnorm` 替换回 MLP，也不要用旧 adapter 生成
revision 5 证据。

当前唯一下一步以 `migration-spec.md` 为准：

```text
实现 Ticket 22 的 sgl_kernel.gemma_rmsnorm capture/replay adapter；
...
Ticket 22 完成后，runbook 才能补入真实命令。preflight 至少要证明：

1. Contract revision 5、`spec-binding-004` 和 `scan-005` 绑定一致；
2. 请求的 operator 等于 `scan-005.selection.active_operator`；
3. HookRegistry 实际包裹的是上面的现有 call-site；
4. rank 0 保存三种 shape，重复 shape 和第四种 shape 不新增样本；
5. payload 允许直接参数 `weight`，但拒绝 checkpoint 和 module state；
6. 新进程能加载样本并调用同一 `gemma_rmsnorm` 接口 self-replay；
7. 比较使用 Contract 固定的
