...
四个 Deterministic Tool 只解析下面两个标记之间的 JSON，不解析 Contract 的其他 Markdown，也不读取或修改 Working State。任何必填值为 `null` 时，本 Contract 尚不可执行。
...
`schema`、`spec_id` 与 `contract_revision` 的唯一机器可读值位于 Contract Data，不在 Markdown 正文维护副本。首次批准前必须填写 `human_owner`、Contract Data 中全部 `null`，把 `contract_revision` 设为 `1`，并记录初始 Human Decision。
...
- SGLang 与 SGLang-Kunlun revision、checkpoint、配置摘要、target/draft 入口和 Demo 输入模式全部由 Contract Data 固定。
